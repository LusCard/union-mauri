...
export const creatorRequest = async (req, res) => {
  try {
    ...
    if (!isSave)
      return res
        .status(404)
        .json({ message: "no se puedo hacer el envió de la petición " });
    ...
    console.log(
      color.red(
        "                          Error al enviar una petición al servidor "
      )
    );
    console.log(
      color.blue(
    ...
    return res.status(404).json({
      message: "No se pudo hacer la petición por favor intente mas tarde ",
    });
  }
};
...
export const acceptRequest = async (req, res) => {
  try {
    ...
    if (!isValid)
      return res
        .status(400)
        .json({ message: "La id de los parámetro no es valida " });
    ...
    if (!theRequest)
      return res
        .status(404)
        .json({ message: "no se a encontrado la petición " });
    ...
    if (!reqDeleted)
      return res.status(304).json({
        message: "no se pudo eliminar la petición por favor intente de nuevo ",
      });
    ...
    console.log(
      color.red(
        "                     Error en el controlador de aceptación de pericones "
      )
    );
    console.log(
      color.blue(
...
export const denialsRequests = async (req, res) => {
  try {
    ...
    if (!isValid)
      return res
        .status(400)
        .json({ message: "la id del parámetro es invalida" });

    const reqDeleted = await request.findByIdAndDelete(id);

    if (!reqDeleted)
      return res.status(304).json({
        message: "No se a podido eliminar la petición por favor intente nuevo ",
      });
    ...
    console.log(
      color.red(
        "                     Error en el controlador de negación de peticiones "
      )
    );
    console.log(
      color.blue(
